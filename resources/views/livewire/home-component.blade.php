            ...
            <div class="slide-carousel owl-carousel style-nav-1" data-items="1" data-loop="1" data-nav="true" data-dots="false">
                @foreach ($sliders as $slide)
                    <div class="item-slide">
                        ...
                        <div class="slide-info slide-{{ $slide->type_slide }}">
                            <h2 class="f-title"><b>{{ $slide->title }}</b></h2>
                            <span class="{{ $slide->type_slide == 1 ? '' : 'f-' }}subtitle">{{ $slide->subtitle }}</span>
                            <p class="sale-info">{{ $slide->type_slide == 1 ? 'Stating at: ' : 'Only price: ' }}<span class="price">{{ currency_IDR($slide->price) }}</span></p>
                    ...
                    @foreach ($sproducts as $sproduct)
                        <div class="product product-style-2 equal-elem ">
                            ...
                            <div class="product-info">
                                ...
                                <div class="wrap-price"><ins><p class="product-price">{{ currency_IDR($sproduct->sale_price) }}</p></ins> <del><p class="product-price">{{ currency_IDR($sproduct->regular_price) }}</p></del></div>
                            </div>
                        </div>
                    @endforeach
                                ...
                                @foreach ($lproducts as $lproduct)
                                    <div class="product product-style-2 equal-elem ">
                                        ...
                                        <div class="product-info">
                                            ...
                                            <div class="wrap-price"><span class="product-price">{{ currency_IDR($lproduct->regular_price) }}</span></div>
                                        </div>
                                    </div>
                                @endforeach
                            </div>
                    ...
                    <div class="tab-control">
                        @foreach ($categories as $key=>$categ)
                            ...
                            @if ($c_products->count() > 0)
                                <a href="#category_{{ $categ->id }}" class="tab-control-item {{ $key == 0 ? 'active':''}}">{{ $categ->name }}</a>
                            @endif
                        @endforeach
                    </div>
                    <div class="tab-contents">
                        @foreach ($categories as $key=>$categ)
                            <div class="tab-content-item {{ $key == 0 ? 'active':''}}" id="category_{{ $categ->id }}">
                                <div class="wrap-products slide-carousel owl-carousel style-nav-1 equal-container" data-items="5" data-loop="false" data-nav="true" data-dots="false" data-responsive='{"0":{"items":"1"},"480":{"items":"2"},"768":{"items":"3"},"992":{"items":"4"},"1200":{"items":"5"}}' >
                                    @php
                                        $c_products = DB::table('products')->where('category_id', $categ->id)->get()->take($no_of_products);
                                    @endphp
                                    @foreach ($c_products as $c_product)
                                        <div class="product product-style-2 equal-elem ">
                                            ...
                                            <div class="product-info">
                                                ...
                                                <div class="wrap-price"><span class="product-price">{{ currency_IDR($c_product->regular_price) }}</span></div>
                                            </div>
                                        </div>
                                    @endforeach
